...
const Post = ({ post }) => {
  ...
  const hasLiked = likes.some((id) => id.toString() === UserData._id.toString());
    ...
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="w-10 h-10 rounded-full bg-gray-300 overflow-hidden">
          {post.author?.picture ? (
            <img src={post.author.picture} alt="Author" className="w-full h-full object-cover" />
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">{post.author?.headline || "User"}</p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center text-gray-500 text-sm pt-2 border-t">
        ...
        <button
          ...
          className={`flex items-center gap-2 transition ${hasLiked ? "text-blue-600 font-semibold" : "hover:text-blue-500"}`}
        >
          <ThumbsUp size={18} className={hasLiked ? "fill-blue-600 text-blue-600" : ""} />
          Like
        </button>
